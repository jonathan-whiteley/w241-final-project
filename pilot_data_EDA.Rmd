...
We can get an idea of the values in the table with a summary of the statistics for each column. As we have seen here, people spent 27 seconds at a minimum to finish the survey in the pilot whereas we tested the survey ourselves. Control group took us about 40 seconds and test groups took us about 70 seconds. We worry that this finding might be suggesting that some subjects may not be actually getting the treatment when they were given the treatment (i.e. non-compliance). Therefore, we decided to implement timer in the production survey.
